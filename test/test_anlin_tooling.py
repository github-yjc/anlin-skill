...
class AnlinToolingTests(unittest.TestCase):
    ...
    def test_formal_first_draft_uses_source_loop_not_long_repair_files(self) -> None:
        ...
        self.assertIn("clean-generation-brief.md` is the detailed clean-eval repair interface", skill)
        ...
        self.assertIn("`references/clean-generation-brief.md` and `references/runtime-brief.md` are not harmless supplements before the first draft", skill)
        ...
        self.assertIn("The first complete draft should start from `references/clean-eval-first-draft-minimum.md`", clean)
        ...
        self.assertIn("do not open `references/clean-generation-brief.md`", skill)
        ...
        self.assertIn("do not glob, search, list parent skill directories, reason aloud about finding the skill directory", skill)
        self.assertIn("In ordinary repair, ask for the skill directory only if the user wants deterministic validation", skill)
        self.assertIn("In formal finalized repair, do not ask or search during the attempt", skill)
        ...
        self.assertIn("Do not enumerate lines", skill)
        self.assertIn("let `clean_run_checker.py` do exact counting", skill)
        self.assertIn("These numbers are a visual corridor, not a pre-write arithmetic task", skill)
        ...
        self.assertIn("`后面跟着...`", skill)
        self.assertIn("he said OK -> I returned to a wet sleeve / water heater / balance screen", skill)
    ...
    def test_runtime_docs_distinguish_clean_eval_from_ordinary_use(self) -> None:
        ...
        self.assertIn("target roughly 55-65 body lines", runtime)
        ...
        self.assertIn("Do not search parent or sibling skill directories to find that reference", skill)
        self.assertIn("Inline fallback if the reference path is not directly available", skill)
        self.assertIn("still write the revised artifact and mark local validation unresolved", skill)
        self.assertIn("do not exit with an unchanged draft", skill)
        ...
        self.assertIn("full profile report is controller evidence after the artifact is frozen", skill)
        finalized_lines = [line for line in skill.splitlines() if "Finalized repair / formal evaluation mode" in line]
        ...
        self.assertIn("do not read or grep checker source, tests, or hidden threshold constants", skill)
    ...
    def test_skill_load_order_keeps_background_as_post_scene_fact_gate(self) -> None:
        ...
        self.assertIn("references/clean-eval-first-draft-minimum.md", always_start)
        self.assertIn("references/clean-generation-brief.md", skill)
    ...
    def test_style_profile_repair_brief_is_compact_generator_interface(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("standard_prewrite_shape_check: before the single atomic write", result.stdout)
            self.assertIn("standard_punctuation_balance_guard: line-final comma is not a target", result.stdout)
            self.assertIn("standard_long_row_guard: several rough long rows must be visible", result.stdout)
            self.assertIn("standard_message_order_guard: in invitation/refusal repairs", result.stdout)
            self.assertIn("standard_rough_exposure_guard: at least one repaired cluster should expose", result.stdout)
            self.assertIn("standard_public_hinge_guard: a neighbor/cashier/rider appearing is not enough", result.stdout)
    ...
    def test_finalized_repair_docs_route_profile_brief_and_full_report_separately(self) -> None:
        ...
        layer = (ROOT / "references" / "runtime-layer-map.md").read_text(encoding="utf-8")
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        combined = "\n".join([skill, runtime, finalized_minimum, validation, layer, readme])
        ...
        self.assertIn("only one or two rows run long enough", finalized_minimum)
        ...
        self.assertIn("If the planned rewrite has zero obvious long rows, or if only one or two rows", finalized_minimum)
    ...
    def test_runtime_source_docs_guard_comma_carpet_and_public_hinge_cameos(self) -> None:
        ...
        self.assertIn("75-80+ body lines", skill)
