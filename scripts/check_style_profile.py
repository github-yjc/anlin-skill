...
def format_repair_brief(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "hard_gate_pass_preservation: if the preceding hard gate already passed and this is only a style-profile review, do not clean the draft into a new hard-gate failure. Use micro_cluster_surgery with line_ending_lock and mass_floor_lock: preserve complete standard-diary mass, natural connector spread, rough/public consequence, mixed comma/hard-stop line endings, and preserve working comma-ended continuation rows from the incoming draft while doing local source surgery; keep row-ending punctuation and line breaks for untouched rows; do not remove a functional consequence sentence unless you replace it inside the same local cluster; do not add a new simile, analogy, or caption, and do not make line-final comma ratio zero by closing every row with `。`. A repair that introduces `高频词覆盖不足`, `标准日寄句号网格`, a below-900 shrink, or a one-period-per-row surface is worse than the original review.",
