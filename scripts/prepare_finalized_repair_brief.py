...
def format_brief(
    *,
    ...
    lines = [
        ...
        *(
            [
                "hard_gate_pass_preservation: current artifact already passed the strict hard gate. Treat profile repair as micro_cluster_surgery with line_ending_lock, not cleanup. Preserve connector spread, complete body mass, rough/public consequence, existing comma continuations, mixed line endings, and preserve working comma-ended continuation rows from the incoming draft; keep row-ending punctuation and line breaks for untouched rows; do not rewrite a review artifact into `高频词覆盖不足`, `标准日寄句号网格`, below-900 shrinkage, line-final comma ratio zero, or a one-period-per-row surface; do not add a new simile, analogy, or caption to explain pressure.",
