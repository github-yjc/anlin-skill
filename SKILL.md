...
1. Run `scripts/check_anlin_violations.py <draft>`.
   ...
   - Use at most one checker-driven repair loop inside the generation agent. If the second local draft still has soft rhythm warnings, connector warnings, comma-ratio warnings, breathing-point warnings, or other corpus-calibrated style warnings, output the clean best article only. The external controller will validate it.
