...
For short sincere family or Mother's Day prompts, choose the present-action anchor before choosing the memory. The anchor is one current action that would still exist without the prompt and changes the next move: a sink or bowl problem, a door/neighbor interruption, a bad or unsent reply, a body or clothing embarrassment, a room chore, a call, or a small object failure. Do not let `eggs -> childhood rain -> no blessing -> final residue` become the article's spine. The memory should leak from the action; it should not arrive first and then recruit present objects as proof.
