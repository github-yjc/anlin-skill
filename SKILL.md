...
5. In clean-eval mode, run `scripts/clean_run_checker.py draft.md --strict --draft-gate`. It delegates to the normal checker and enforces the two-call limit. Calling this wrapper commits the current `draft.md` to clean-eval protocol; do not later reinterpret the same run as ordinary user mode. If it prints `CLEAN_RUN_PREFLIGHT`, the draft was not ready and no checker call was consumed; revise from the preflight message and the already-loaded references, then run the wrapper again. If the preflight says `body_lines < 45` or `prose_block_shape=compressed`, first run `scripts/split_long_lines.py draft.md --in-place --target-lines 58`, inspect once, then repair content; if it says `body_lines > 90`, `short_line_grid`, or `long_lines < 4`, run `scripts/merge_short_lines.py draft.md --in-place --target-lines 68` or manually merge adjacent action/speech/thought rows. Do not rewrite the same prose as 100+ tiny rows, and do not create line breaks by deleting punctuation. Do not call `check_anlin_violations.py` directly, grep/read checker source, or browse tests to discover hidden tokens after a preflight. If it prints `CLEAN_RUN_PREFLIGHT_STOP`, the draft stayed unready after the bounded preflight attempts; stop repair work, read `draft.md` once, and output it unchanged so the controller can mark the clean-eval run invalid or failed. Do not switch to the normal checker after this. If the first actual call reports prose compression, run `scripts/split_long_lines.py draft.md --in-place --target-lines 58`; if it reports short-line grid or uniform rhythm, run `scripts/merge_short_lines.py draft.md --in-place --target-lines 68`; if it reports `行末逗号比例`, run `scripts/soften_line_endings.py draft.md --in-place`. Then repair content once.
