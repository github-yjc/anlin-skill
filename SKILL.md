...
3. Run `scripts/clean_run_checker.py draft.md --strict --draft-gate` for formal generation checks. It delegates to the normal checker and enforces the two-call limit. If it prints `CLEAN_RUN_PREFLIGHT`, the draft was not ready and no checker call was consumed; continue writing concrete material, then run the wrapper again. If the first actual call reports prose compression, run `scripts/split_long_lines.py draft.md --in-place --target-lines 58`; if it reports short-line grid, run `scripts/merge_short_lines.py draft.md --in-place --target-lines 68`; if it reports `行末逗号比例`, run `scripts/soften_line_endings.py draft.md --in-place`. Then repair content once.
