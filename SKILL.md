...
6. If you write `draft.md` again after any rhythm script, the previous rhythm repair no longer counts. Before the second checker, run `rebalance_line_rhythm.py` when the current file has fewer than 45 body lines, more than 75 body lines, fewer than six natural long lines, visible long prose blocks, or a short-line grid. Run `soften_line_endings.py` again when the first 20 body lines mostly end in `。`. The target is a middle rhythm corridor, not perfect ratios.
