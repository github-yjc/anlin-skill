...
- **Finalized repair mode**: the controller copied a bounded draft into a `finalized/` directory. If `repair-brief.txt` declares either compact mode, read only `draft.md` and `repair-brief.txt`.
  ...
  - In `source_rewrite_compact`, follow the one source focus and exact scope supplied by the brief. Most routes use one local cluster; a severely underbuilt route may explicitly require a source-level rebuild across the existing movements. Do not reconstruct hidden blocker/profile lists or add an independent scene/packet. Make the one complete write to `draft.md`, then stop. If a chat reply is required, output exactly `artifact_written`.
