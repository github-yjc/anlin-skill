...
3. Run `check_anlin_violations.py draft.md --strict --draft-gate` at most twice. If the first checker reports prose compression, run `scripts/split_long_lines.py draft.md --in-place --target-lines 58`; if it reports short-line grid, run `scripts/merge_short_lines.py draft.md --in-place --target-lines 68`; if it reports `行末逗号比例`, run `scripts/soften_line_endings.py draft.md --in-place`. Then repair content once.
