...
1. Check `.anlin-clean-eval-mode` as one direct tool action, preferably `Test-Path -LiteralPath ".anlin-clean-eval-mode"` (a directory listing that merely shows the filename is not a marker check).
...
- Use punctuation-bearing movement units: keep an observation, crooked read, and next action connected with the natural punctuation they need. A line break does not remove punctuation; naked caption rows are not breathing rows. Do not split every sentence into its own line. Keep short clauses attached to the action, reply, object, or thought they complete. Punctuation inside a long row is not lineation. When the next row completes the same movement, let the previous row keep the natural comma or unfinished landing that carries it forward. Do not hide every continuation inside long prose rows, and do not seal every visible row with a period. Define an independent thought-turn by a change in object, time, memory, joke, social position, or next action, not by paragraph or row count; a rhythm script cannot invent missing source movement.
- A complete standard diary is a day-shaped collage, not a premise summary: if the body only has the prompt surface, one memory, and a final reply, continue through another independent thought-turn before the required residue.
- For linear social/event prompts, treat the prompt sequence as a fact constraint, not the article's outline: start with a non-prompt fragment, crop the event to one needed surface, leave it for an independent thought-turn, and return to any required reply only as a short residue.
- For social-decline prompts, treat the invitation or refusal as one possible fragment. Reject a single carrier chain from present desk/room through message, ticket/money, old-friend inventory, work excuse, refusal, and friendship thesis; replace one middle fragment with a lateral thought instead. Do not invent a consequence chain, group-chat crowd, work biography, or etiquette closure merely to prove the prompt.
