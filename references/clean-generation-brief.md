...
A complete standard diary can still fail when it becomes 50-70 finished `。` rows. Before writing `draft.md`, scan the page shape: if most lines are complete statements and almost none are <=8-character drops, rebuild clusters rather than changing punctuation. Let one action run downward through `，`, let one rough line stay long because the hand, reply, payment, or body has to keep moving, then let a short drop land a bad answer, failed decision, or low body fact. Do not solve this by adding another paragraph or by making every line comma-ended.
