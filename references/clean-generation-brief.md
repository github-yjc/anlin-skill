...
- `CLEAN_RUN_POSTCHECK_PREFLIGHT`: the wrapper left the submitted draft unchanged. For a pure shape result, do not add a new scene or consequence cluster; perform only the explicit local rhythm action printed by the wrapper. For an underbuilt or source-blocked result, repair by replacement before the final checker without shrinking the article or appending a new proof packet. For invitation, wedding, reunion, or other social-decline prompts, the replacement must make the refusal change the next visible action instead of adding another private screen, water, room, or etiquette line.
