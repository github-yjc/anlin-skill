...
- no high-signal opening inventory: the first body lines should not list `情人节/朋友圈/晒花/转账/红包/玫瑰/礼物/香菜/备注` before a practical source action exists
...
- no soft witness-only handoff: `看了我一眼` / `往下看了一眼` must alter payment, reply, bag/object state, dirty hand/clothing exposure, door movement, or next action, not only let the narrator privately notice a stain
