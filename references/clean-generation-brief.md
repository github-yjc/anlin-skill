...
For illness or body-pain prompts, do not build the article as symptom -> search page -> refrigerator inventory -> abstract label -> ambient sound. Pick one symptom surface, one cropped screen wording if needed, then leave through a practical or social consequence: a failed walk to the kitchen, a bad reply, a neighbor/cashier/doorway exposure, a spill, a bathroom/slipper problem, a small errand, or an object that forces the body to move. Pain, food taboos, and search results are pressure surfaces, not the whole scene slate.
...
Near-miss drafts are a repeated failure. If the candidate feels complete but has only 35-44 actual body rows, or roughly 850-949 Chinese characters with only two connector families, do not save it and hope preflight will expand it. Add one full off-axis life cluster before writing: 6-10 visible lines where a body interruption, neighbor/cashier/roommate line, small errand, dirty object, payment action, or screen residue changes the next action. That cluster should naturally create two new turn traces such as `发现/不过/突然/于是/因为/所以`. Do not add one explanatory paragraph, one extra symptom, or a decorative short line.
...
- `short_genre_literary_story_closure`: do not stretch the short form into a standard diary. Break the neat title-object/memory/message/ending chain by cutting one symbolic echo or sentimental proof scene, then add a present-day interruption, awkward reply, or useless residue already implied by the scene.
