...
Use this file after the first clean-eval `draft.md` exists, when the wrapper reports preflight/checker findings or the controller needs the detailed clean-eval contract. The first complete draft should start from `references/clean-eval-first-draft-minimum.md`; this file preserves expanded tool-flow, genre, and repair guidance without becoming the pre-draft checklist.
...
Keep the narration in first person unless another person is clearly being quoted or described. A line like `他把手机塞到枕头底下` after a first-person diary turns the draft into a short story; rewrite it as the narrator's own action or delete that story-like tail.
...
Clean-eval first-draft tool order is part of the test, not a suggestion: marker check -> current-directory confirmation -> read `clean-eval-first-draft-minimum.md` -> for standard diary read `standard-diary-source-engine.md` -> write one complete `draft.md` -> run `clean_run_checker.py`. The repair step "read this brief" means using the reference file path supplied by the already-triggered skill after the wrapper reports a finding; it never means searching parent skill directories to locate the skill again. After this brief is loaded for repair, do not draft trial paragraphs, scene plans, or repeated scratch versions in the terminal. If the article is imperfect, persist the best complete titled candidate to `draft.md` and let the wrapper diagnose it; a visible scratch article without `draft.md` is a failed run.
...
Before the first complete `draft.md`, do not read this file as an extra source checklist. Use `clean-eval-first-draft-minimum.md` plus `standard-diary-source-engine.md` for standard diary clean-eval. After this file is loaded for repair, stop reading references except the smallest file needed by the reported failure. `runtime-brief.md` is not a harmless supplement before the first draft. If you want more guidance, use the source loop below and write the complete `draft.md`; deeper files are only for repair or controller analysis after the first wrapper pass.

For standard diary clean-eval, load `references/standard-diary-source-engine.md` after the short minimum file and then write `draft.md`. That file is part of the minimal first-draft pack. It is shorter than this reference on purpose: use it to build the middle from consequence kernels, not as another checklist. Do not run rhythm scripts before the first wrapper call.
...
Do not write a prose version first and then promise to restructure it. In clean-eval mode, the first persisted `draft.md` is already part of the measurement. If the draft in your head is 8-15 paragraphs, or 16-25 dense rows where the row breaks do not come from action/reply/body/payment movement, stop before writing and rebuild it as breathing clusters.
...
Do not treat `practical consequence` as permission to invent a current office life. Unless the user supplied it, avoid consequences built from a leader, company group, office file, Monday deadline, leave request, half-day leave, `请不下来`, 调休, 排班, attendance, salary penalty, or current work shift. Lower that pressure to a cropped screen surface, old coworker or someone else's company, a generic message, neighbor/cashier contact, route problem, bathroom/slipper trouble, or a body action that directly blocks the day.
...
Known failed source shape: a fluent 10-15 paragraph article, or a 16-25-row dense article, later line-broken by tools into 55-70 lines, still tends to keep prose logic, period-heavy rhythm, repeated connector glue, and binary reframe sentences. Do not use that path. Before saving the first `draft.md`, the visible article itself should already look like clusters of breath: one or two lines may run on with `，`, a longer action/speech/thought line may carry the load, then a short drop lands a bad reply, body lowering, failed decision, or practical retreat. The short drop is earned by the scene; it is not a caption row.
...
Near-miss drafts are a repeated failure. If the candidate feels complete but visibly lacks a full middle corridor or uses only two obvious turn traces, do not save it and hope preflight will expand it. A standard-diary candidate around 600-850 body Chinese characters is usually not restrained; it is an underbuilt source even if it already has 45-60 lines. Add one off-axis life cluster before writing: a body interruption, neighbor/cashier/roommate line, small errand, dirty object, payment action, or screen residue changes the next action. That cluster should naturally create new turn traces such as `发现/不过/突然/于是/因为/所以`. Do not add one explanatory paragraph, one extra symptom, or a decorative short line.

Rough self-damage is narrower than ordinary awkwardness. A neighbor looking once, a face "probably looking bad", or a line saying the narrator "seems stupid" is usually still tasteful narration. The wound should change the body, social position, reply, route, money, or next action: dirty hands/clothes get noticed by a cashier or neighbor, a stained thing becomes visible under a hallway light, the body makes an embarrassing sound or cost, a reply comes out wrong, the narrator is mistaken for something ridiculous, or a practical low-status maneuver becomes visible. Do not separate the witness and the ugly fact into "someone looked" followed by "I privately noticed my stain"; that still leaves the shame inside the narrator. The other person's look, pause, payment hand, door movement, or reply should touch the ugly fact in the same chain or force a changed action. Ordinary unclean consequences also work when they lower the narrator by action or social exposure: slippers worn on the wrong feet, snot nearly coming out, pants dirtied and noticed, sticky drain hair, socks worn through, a public stomach noise, or someone reacting to the narrator's dirty hand. Do not add shock or treat these as required tags; make the existing scene lower the narrator by consequence.
...
For hallway or stairwell witnesses, keep the low-status chain plain and continuous. A useful line is not "阿姨看了我一眼"; it is the neighbor/aunt naming the leak, smell, dirty sleeve, blocked door, or floor mark while the narrator is still wiping, letting them pass, closing the door, failing to answer, or moving the body badly. A neutral question about the heater, order, delivery, or balance is still a cameo if it does not touch the wet/dirty/payment/reply/body fact and force the next move. Opening the door, seeing corridor light, noticing light under a neighbor's door, and closing the door fast are still threshold atmosphere unless they force contact, payment, reply, route, or body action to change. After that, switch medium; do not keep proving the same sleeve, stain, oil, or bag for the rest of the article.
...
Before the first body line, choose 2-3 consequence kernels in your head. A kernel is valid only when it has a consequence verb: somebody/something waits, asks, points, holds, returns, fails, leaks, spills, closes, drops, wipes, hides, pays, answers, leaves, or changes route, room, reply, body state, payment, or the next action. A camera verb is invalid by itself: look, glance, sweep, notice, nobody says anything, light stays on, sound continues, or the narrator privately inspects a stain. If the kernel is `rider looks -> I notice pants/sleeve -> rider leaves`, rebuild it before drafting. If the kernel is `bag leaks -> rider waits at the door -> I wipe my hand on pants and close the door too fast`, it can carry movement.

Do not make this a visible plan. It is a private source gate. The saved article should show the consequence through action, not through labels such as `我很丢人` or through a checklist of contact/payment/body/object. The final line should also inherit one earned consequence verb: an unfinished chore, wrong object, payment/reply/route/body interruption, or a stupid next move. Do not end by fading out on light, wind, appliance noise, screen glow, or the room still being quiet unless that ambient fact actively forces the next action.

For delivery, cashier, neighbor, shopkeeper, or hallway contact, do not write a gaze line first and then try to repair it. Avoid sentences built around `看了我一眼`, `往下看`, `瞥了一眼`, `扫了一下`, or `没说什么`. Start the contact from the changed object or transaction instead: the bag leaks, a hand waits, a phone refuses to scan, the door closes wrong, the person asks/points/returns/holds, or the narrator wipes, drops, hides, fails to answer, or changes the next move. If a line can be reduced to `someone looked and left`, delete it before `draft.md`.

For standard diary, expand the loop into 2-3 load-bearing action clusters before the first `draft.md`. A load-bearing cluster is not a topic scene or a style label. It is a short movement whose removal would break what happens next because it changes action, body state, reply, route, payment, room position, or social position. In simpler terms, deleting it would break the next action, not merely remove color. At least one cluster should be independent of the prompt's most obvious object or screen surface, and at least one should expose the narrator through a practical handoff, outside contact, bad reply, body lowering, or low-status cost. These clusters should naturally create several longer action/speech/thought lines and connector turns; do not add them as a late paragraph after a phone/order/bed chain. Do not build clusters as one finished observation per row. A usable cluster has a before/after movement: the door opens too fast, a payment or reply stalls, a bag leaks, a hand wipes on pants, a person waits, or an object forces the next action.

Use this private source test for quiet standard prompts: if the draft can be summarized as `phone/feed -> order food -> wrong item -> wash bowl -> bed`, restart before saving. A delivery handoff, hallway light, mirror, stain, or leaking bag only counts when it changes the next move or social position. If deleting it leaves the same story, it is decoration, not a load-bearing action cluster. For delivery/order scenes, a rider or cashier who only looks once and leaves is still decoration. Make the person wait, ask a plain question, point at a leak, hold the bag, watch a failed payment, cause a bad reply, or force the narrator to close the door, wipe a hand, drop something, or change the next room action.

For private-holiday or social-feed prompts, the first 8-12 body lines must belong to a body, room, charger, door, payment, sink, cold water, broken object, or other useless action that would still exist without the assignment. Do not open by listing `情人节/朋友圈/晒花/转账/红包/玫瑰/礼物/香菜/备注` or a feed inventory such as `有晒花的，有晒转账的，有晒电影票...`, `满屏都是花和转账截图`, or `朋友圈里全是康乃馨、合照、红包`. A one-line inventory is still an inventory. Let one cropped feed or order surface leak after the first practical action has already made the room/body/app crooked, and make that surface change the next action rather than decorate the prompt.
...
- 2-3 load-bearing action clusters for standard diary, not one quiet chronology
...
For invitations, weddings, reunions, job announcements, or other social-comparison message prompts, the chat log is not the article. Before drafting, pick a side action that could exist without the invitation: a bowl that will not wash, a charging cable under a chair, a bad payment screen, a dirty sleeve, a route search, a doorway errand, or another low practical handle. Also pick the post-refusal consequence before writing: the other person's reply changes the next action, the narrator sends the wrong-size lie, the payment/route decision fails, an old debt interrupts the hand, or a practical embarrassment is exposed. In this prompt family, one load-bearing kernel must be the refusal aftermath itself; the side object may open the door, but it cannot replace the social consequence. The consequence should be plain, not gift-wrapped: do not let the other person send a small red packet, `沾点喜气`, `心意到了`, or `项目忙就算了` as a tidy kindness after the refusal. Use silence, one ordinary reply, an address/photo that worsens the decision, payment/route/body trouble, or a remembered debt that changes whether the narrator answers. If the remembered debt becomes 转账/备注/已读/没领/未领取 only, it is still a private screen loop; it must force a visible reply, a body or doorway problem, a route/payment consequence, or disappear. The invitation enters only after the side action has already made the room/body/app crooked. Do not title the article with the person plus event, such as `狗哥的婚礼`, and do not let the first meaningful line restate the whole prompt. Use at most one cropped message surface, keep the money calculation short, and force one off-chat consequence before the middle ends: a worse reply, unpaid food, a route/payment decision, a body interruption, an errand, someone noticing a practical embarrassment, or the other person's response changing what the narrator does next. If the draft is only ticket price -> gift money -> refusal -> venue photo -> old memory, it is a sealed short story and the prompt is still too visible. Old classmate memory should be one cropped debt or object, not a full warm proof-chain that explains why the refusal is sad. Conversely, do not let the side room object eat the article: one cold room, noodle, dirty hand, phone heat, charging cable, delivery box, or washing-hand packet is enough. If 暖气、冷、手指、泡面、水龙头、厨房、洗手、手机发烫、充电线、快递盒, or 袖口 keeps returning in the opening, middle, and ending, the repair has replaced social pressure with indoor texture. Cut that packet and make the reply, money, old relation, route, or other person's reaction change the next action. Before saving, privately delete all room texture except one object in your head; if the social refusal no longer moves, rebuild from the reply aftermath. If the same side object supplies the title, opening disturbance, and final image, break one link before saving; the object is a handle, not a closure machine.

For those social-decline prompts, make a visible refusal chain. It is not enough to show calculation and then send `去不了`. After the refusal, the other person's plain response, the deleted reply, the failed transfer, the old debt, the address/table/seat change, or a body/door/payment interruption must cause the narrator's next action. If the post-refusal section can be summarized as `he said OK -> I felt bad -> I returned to the room object`, the social consequence is still too thin. An `OK` reply or silence is not enough unless it makes the narrator do something lower or more exposed while the pressure is still live: a wet/dirty hand changes the reply, somebody waits at the door, payment/route stalls, or a body/door/object problem changes the next move. A plain OK reply plus a private screen mark is still a screen loop if the mark only proves the hand was wet. Spend the next movement on what the refusal did to the hand, reply, payment, route, or status; return to a bowl, sleeve, water, charger, or paper only if that object is now changed by the refusal. Do not move the plot by message order such as `X发了第二条`, `第二条只...`, `又发来一条`, `下面还有一条`, `下面还有一个...`, or `后面跟着...`; choose one cropped screen surface and make it alter hand, reply, payment, route, body, or door action.

Do not overcorrect a thin refusal chain into group-chat exposure. `群里有人问`, `有人@我`, `你怎么说项目忙`, `项目不是结了吗`, `正在输入`, and similar public crowd cues are still screen summaries if they do not change one local action. Cut them and use a smaller consequence: one person waiting/asking, a payment or route stall, a wet/dirty hand changing the phone action, a smaller wrong reply, or a body/door/object problem while the refusal pressure is live.

Do not close the refusal through narrator etiquette. A red packet apology, `抱歉人不到钱到`, `人不到没事`, `下次一起吃饭`, `心意到了`, or a clean final `好` often turns the draft into a solved moral exchange. Keep money as hesitation or unfinished action, or remove it; leave through wrong reply, route/payment residue, old debt, body/door trouble, or a low object that does not settle the relationship.

Refusal chain upper bound: chain means one consequence transfer, not plot continuation. Keep it on the same evening or one tiny next-day residue. One refusal consequence may cause one or two visible actions, then exit. Do not extend into several days, class-group photos, commute scenes, coworker commentary, office biography, wedding logistics,伴郎/喜酒 turns, or another person's neat gift/reversal. When a social-decline draft grows past the length corridor, cut chronology first, not the roughness; do not add more room objects or punctuation repairs around an overgrown timeline.
...
For quiet interior prompts, especially phone/feed + room + food/order + bed, add one outside contact before the first `draft.md`, but do not treat contact as a cameo. The contact can be tiny: a delivery handoff, hallway or stairwell interruption, cashier/neighbor line, leaking bag, wrong reply, or object mark that becomes visible under a light. It must be load-bearing: it changes the next action, leaves a physical residue, lowers the narrator in front of the world, or makes a later reply/payment/body move happen. A single glance from another person, a mirror face, a hallway light, `他看了我一眼，大概看到...`, or `我看起来挺差` is too soft when the article would read the same without it. If the look does not make the narrator close the door too fast, drop the bag, fail to answer, wipe a hand, change payment/reply/room position, or otherwise alter the next action, delete it or rebuild it. Do not let the scene become `rider looked -> I privately noticed pants/sleeve -> rider left`; that keeps the shame private and leaves the article as phone/food/bed with a camera cut.
...
- no high-signal opening inventory: the first body lines should not list `情人节/朋友圈/晒花/转账/红包/玫瑰/礼物/香菜/备注` before a practical source action exists; `满屏都是花和转账截图` is still prompt inventory even though it is one sentence
...
- no soft witness-only handoff: `看了我一眼` / `往下看了一眼` / `扫到油渍` must alter payment, reply, bag/object state, dirty hand/clothing exposure, door movement, or next action, not only let the narrator privately notice or rub a stain
- standard diary has 2-3 load-bearing action clusters; at least one is not the prompt's obvious screen/order/object, and deleting any one cluster would damage what happens next
...
When a draft has 45-70 lines but no long action/speech/thought rows, it is usually one caption per line. A 45-70-line page where every row is only 10-18 Chinese characters is still a caption grid, even if it avoids both 8-15 prose paragraphs and 80+ tiny rows. Before saving, merge some adjacent object facts into rough rows that carry both the thing and its consequence; several rows should visibly run beyond about 24 Chinese characters because action, speech, or thought is still moving. For example one line can contain seeing the old slipper, the hallway light, and what that exposure does to the next action. Do not break every observed fact into its own sentence row.
...
For 朋友圈, short-video feeds, annual-summary feeds, new-year flag screens, old-chat records, or romantic/social comparison prompts, one surface is enough before the first checker. Do not write a feed montage: three posts, a caption quote, a like/unlike gesture, and a final latest video usually reads as prompt coverage. Do not open with a feed inventory such as `有晒花的，有晒转账的，有晒电影票...`, `满屏都是花和转账截图`, or `朋友圈里全是康乃馨、合照、红包`; that is the prompt wearing a diary mask. Do not replace a long feed list with a shorter feed list. Pick one cropped thing on the screen, then move away from the feed into a body, food, money, room, route, reply, outside contact, or object consequence. Do not write a screen-archaeology chain either: 朋友圈年度总结 -> 2021聊天记录 -> 群聊 -> 通讯录 -> 外卖推送 is still a neat assignment path even when each surface is plausible. If the feed or old record returns later, it must change action or social position, not add another example of the same pressure.

For private holiday, romance-feed, takeout, wrong-food, or lonely-room prompts, do not let the source become `phone/feed -> order food -> wrong item -> wash bowl -> bed`. That is a tidy prompt-shaped short story even when every detail is concrete. Before the first `draft.md`, choose one side engine that would still happen without the holiday or romance prompt: a payment stall, door or hallway exposure, rider/shopkeeper/neighbor contact, water/sink failure, dirty clothing under public light, wrong reply, small errand, or object leak. Make it a load-bearing action cluster, not a prop: the rider contact should force a door/hand/room/payment/reply consequence; the sink should interrupt the body or next move; the wrong reply should change what the narrator does. A rider, cashier, neighbor, or stranger who only notices oil, sleeves, bare feet, or a messy room and then disappears is not an engine; decide the consequence verb before drafting: wait, ask, point, return the bag, make payment fail, force the door to close wrong, make the narrator drop/wipe/hide something, or change the next room action. Use at most one feed/gift surface and at most one wrong-food surface as visible pressure; crop, bury, or discard the rest. If the prompt combines romance-feed pressure with wrong-food/order pressure, one of those surfaces must be weakened before drafting. Do not let title, body spine, and tail all point to `备注/备注栏/香菜/朋友圈`. A standard diary titled `备注`, `备注栏`, `香菜`, `礼物`, `玫瑰`, `优惠券`, `朋友圈`, or another prompt prop is not automatically wrong, but it fails when the prompt packet keeps proving itself across the article. Retitle from a side consequence such as a sleeve, charging line, door, wet bag, wrong shoe, sink, payment, or room object that changes action; cut one prop echo; and let the middle move through a door, hallway, payment, dirty hand, wrong reply, sink, rider/shopkeeper, or another person's reaction before the prop returns.

The same title-body-tail rule applies to replacement materials. A title such as `油`, `油印`, `油印子`, `袖口`, `碗`, or `手机` is high risk when the body keeps returning to that exact material as evidence. It can survive only if the middle moves away from it and the ending exits through a different practical leftover. Otherwise retitle from an action/position and cut one material echo before drafting or repairing.
...
Draft in breathing clusters, not sentence rows. A cluster can be 2-5 visible lines carrying one action/thought movement: one line may end with `，`, one line may run longer with speech or action, the next lands with `。`, then a short drop or ugly reply. Long lines should come from rough action, speech, or thought doing more work, not from `，` chaining five explanations; 13-18 Chinese characters is usually a caption, not a long movement row. A 45-70 line draft with zero real <=8-character drops or zero rows visibly beyond about 24 Chinese characters is still a medium-row grid; it reads like every thought was normalized to the same length. Do not put a blank line after every sentence only to raise line count. Do not turn every line ending into `，` only to satisfy `行末逗号比例`. If a line break does not change breath, action, reply, body, or thought direction, it is formatting, not rhythm.

A complete standard diary can still fail when it becomes 50-70 finished `。` rows. Before saving a standard diary draft, look at the visible body shape rather than imagining the checker will fix it. Before writing `draft.md`, scan the page shape: if most lines are complete statements and almost none are <=8-character drops, rebuild clusters rather than changing punctuation. Let one action run downward through `，`, let one rough line stay long because the hand, reply, payment, or body has to keep moving, then let a short drop land a bad answer, failed decision, or low body fact. Do not solve this by adding another paragraph or by making every line comma-ended.

Avoid the punctuation pendulum. A red comma-density repair that becomes many `没管。/删了。/没回。/算了。` rows has not improved; it has only swung into a period-row grid. A closed-sentence repair that becomes one huge comma chain has the same problem. Repair the source movement: choose what is still happening after the line, who is still waiting, what hand/payment/door/body action has not finished, and where a bad reply or failed decision actually lands. Punctuation follows that movement.

Use local cluster surgery, not global punctuation repair. For `period_row_grid`, choose 3-5 unfinished action spots and rebuild only those into breathing clusters; do not merge the whole piece into comma-heavy paragraphs. For `逗号密度过高`, delete explanatory tails and restore hard stops where actions actually land; do not answer by making every row short and sealed. The visible improvement should be changed movement, not a new punctuation ratio.
...
If the wrapper prints `CLEAN_RUN_PREFLIGHT`, revise before the first checker call is consumed. If it prints `CLEAN_RUN_POSTCHECK_PREFLIGHT`, the first formal checker has already run, but the wrapper is protecting checker call 2/2 from a known underbuilt source. Treat it as the one bounded source-rewrite chance before the final checker call, not as permission to analyze metrics or switch to ordinary repair. The wrapper message is the public repair interface. Do not inspect checker source or tests for hidden tokens, do not grep `early_comma_ratio`, and do not read `clean_run_checker.py` or `check_anlin_violations.py`; if the message is too hard to satisfy, change the source shape rather than reverse-engineering the metric.

Use the preflight message as a shape diagnosis, not as permission to thrash between prose blocks and tiny grids:

- `body_lines < 45`, `prose_block_shape=compressed`, `body_lines > 90`, `short_line_grid`, or `long_lines < 4`: when this is a pure shape drift, run `python <skill-dir>/scripts/rebalance_line_rhythm.py draft.md --in-place`, read once, then make only local source adjustments. When the same preflight also names connector spread, weak engine, missing rough self-damage, quoted dialogue, prompt-shaped opening, social-decline consequence, or underbuilt source, do not run the rhythm script first. Rewrite the source/content into a complete line-broken article first, then run `rebalance_line_rhythm.py` once as the final shape step before the next wrapper call. Do not let the repair bounce from short-line grid into 30-40 prose lines.
...
- `纹理替代社交不足` or red `ngram_texture`: read the n-gram direction before editing. If n-gram reuse is too low or uniqueness is too high, preserve one plain action or object phrase that genuinely recurs inside the same action chain; do not synonym-scrub repeated words or make every line uniquely worded. If repetition is too high or templates repeat mechanically, cut one whole repeated local packet and replace its function with a social or practical consequence that changes action, reply, route, payment, or room position. Do not add more body/object/screen proof, and do not treat n-gram repair as a vocabulary-replacement pass.
...
- `CLEAN_RUN_POSTCHECK_PREFLIGHT`: repair by replacement before the final checker. Do not shrink the article to remove findings. Add one functional consequence cluster that changes hand, reply, payment, route, body, door, object, or social position and naturally creates connector turns. For invitation, wedding, reunion, or other social-decline prompts, make that cluster refusal-coupled: the refusal must change the next visible action, not just add another private screen, water, room, or etiquette line.

After any rewrite of `draft.md`, prior rhythm script work no longer applies. Do not overwrite a script-repaired file with a fresh 8-15 paragraph draft and then call the wrapper. If a preflight combines `rebalance_line_rhythm.py` shape drift with content/source blockers such as weak connector spread, missing rough self-damage, weak engine, quoted dialogue, prompt-shaped opening, or refusal-consequence failure, repair source/content first and run `rebalance_line_rhythm.py` last before the next wrapper call. If a preflight combines `early_comma_ratio` with a content problem such as missing rough self-damage, weak engine, or underbuilt source shape, repair the content first and run `soften_line_endings.py` last before the next wrapper call. If you need to rebuild content after `rebalance_line_rhythm.py`, `split_long_lines.py`, `merge_short_lines.py`, or `soften_line_endings.py`, rebuild inside the existing breathing-cluster shape or immediately rerun the relevant rhythm script before the next `clean_run_checker.py` call. This is a hard tool-order rule: after a rhythm script, the next wrapper call is valid only if `draft.md` was not rewritten, or if the relevant rhythm script was rerun after the rewrite. Any later `Write draft.md`, `Edit draft.md`, `Set-Content`, or `Out-File` cancels the previous rhythm-script repair, even when the edit only changes content. Reading the file, thinking about the repair, or visually deciding the rhythm looks okay does not refresh it. Expansion after preflight should add 6-10 visible line-broken rows inside the current corridor, not one prose paragraph.
