...
Controller/developer reference only. This file preserves the expanded clean-eval rationale used to maintain source guidance, wrapper messages, tests, and failure analysis. The bounded generator starts from `references/clean-eval-first-draft-minimum.md` plus the selected genre engine, then repairs from the wrapper output alone. Do not load this file inside a bounded clean-eval generator; the extra long context turns related findings into a visible checklist and can cause counting, shrinkage, or timeout loops.
...
Clean-eval first-draft tool order is part of the test, not a suggestion: marker check -> current-directory confirmation -> read `clean-eval-first-draft-minimum.md` -> for standard diary read `standard-diary-source-engine.md` -> write one complete `draft.md` -> run `clean_run_checker.py`. After a wrapper finding, the bounded generator uses that printed result as the complete repair interface and does not read this file. The controller may consult this reference to understand why the wrapper says to replace source movement, preserve mass, or run a named final shape step. A visible scratch article without `draft.md` is still a failed run.
...
The bounded generator must not read this file as either a first-draft checklist or a post-wrapper repair prompt. It uses `clean-eval-first-draft-minimum.md` plus the selected genre engine before the first write, then the self-contained wrapper output after findings. Controllers and developers may use the sections below to review whether those shorter interfaces preserve the intended source rules; they should not route the generator into deeper references from here.
...
When maintaining the bounded protocol, keep `CLEAN_RUN_PREFLIGHT` and `CLEAN_RUN_POSTCHECK_PREFLIGHT` self-contained. The first protects checker call 1/2; the post-check variant protects call 2/2 from a known underbuilt source. The bounded generator revises from that printed result without reading this file, inspecting checker source/tests, grepping thresholds, or switching to ordinary repair. If the message is hard to satisfy, it changes the source shape rather than reverse-engineering the metric.
