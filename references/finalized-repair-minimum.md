...
If a standard-diary hard gate reports `粗粝自毁信号不足`, do not add a curse, shame label, or isolated dirty object. Replace one private room/screen/ledger packet with a low consequence that changes the next action while pressure is still active: a payment or route action stalls, somebody waits or asks a plain question, a wet/dirty hand changes the reply, a doorway/body problem exposes the narrator, or a bad reply makes the next move smaller. This is also the safest way to repair period grids: the ugly consequence creates unfinished action lines, rough longer rows, and short failure drops without tuning punctuation.
