...
- Because the single write is atomic, do a visible body-shape check before saving. Do not write a standard diary that is mostly one short sentence per row, has no moving long action/speech/thought rows, or has 75-80+ body lines. Merge and rebuild the action clusters before the write; after the write it is too late to patch without invalidating the finalized attempt.
