...
| 28 | Breathing fragments are syntax units; a thought-turn is not a paragraph | `references/clean-eval-first-draft-minimum.md`, `references/anlin-collage-source-model.md` | `A thought-turn is not a paragraph`; `breathing fragments, not sentence rows`; punctuation follows movement; a rhythm script cannot invent missing source movement |
