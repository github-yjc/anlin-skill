...
Generator-side preflight is allowed only through `clean_run_checker.py`. `CLEAN_RUN_PREFLIGHT` means the draft was not ready and no formal checker call was consumed. `CLEAN_RUN_POSTCHECK_PREFLIGHT` means checker call 1/2 has run, but the current repaired draft is still too underbuilt or source-weak to spend checker call 2/2; it also consumes no formal checker call and allows only the one bounded source-replacement repair named by the wrapper. `CLEAN_RUN_PREFLIGHT_STOP` means the generator did not reach a checker-ready article within the bounded preflight attempts; record the run as invalid or failed and do not let the same generation agent continue repairing indefinitely. This preserves the distinction between clean-eval mode and ordinary user mode.
