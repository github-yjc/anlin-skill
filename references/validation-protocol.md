...
2. **Finalized repair checkpoint**: copy the bounded checkpoint draft into a separate `finalized/` case directory, then start from that copy and its visible checker results. The generator-facing attempt is deliberately short: run the formal gates, use the repair brief, make one complete source rewrite, persist `finalized/draft.md`, and rerun the gates once. If it still does not pass, record unresolved repair-path evidence rather than letting the same agent keep editing around metrics. A second write/edit to `finalized/draft.md`, a third formal gate sequence, or explicit threshold arithmetic in the same finalized attempt makes the trace invalid controller evidence even when the artifact changes. A controller may start a new separately logged attempt for diagnosis, but the current finalized attempt remains non-pass. Do not read or grep checker source, test files, or hidden threshold constants. Do not glob or list checker source, parent skill directories, or old logs either. A finalized run that inspects `check_anlin_violations.py`, `check_style_profile.py`, `clean_run_checker.py`, `test_anlin_tooling.py`, or threshold names such as `YELLOW_REVIEW_FAMILY_THRESHOLD` is contaminated even if the final draft later passes. Save the final article and full validation reports separately from the bounded checkpoint. This checkpoint answers: can the skill plus its public repair interface converge to a usable final article without metric chasing or source inspection?
...
- During the repair attempt, run `check_style_profile.py <finalized-draft> --draft-gate --strict --repair-brief --genre <selected-genre>` when the bundled profile is available. The repair brief is the generator-facing interface; it folds the long metric list into source actions so the repairing agent does not optimize every ratio one by one. After one source rewrite and one rerun, stop if the result remains non-pass. After the finalized artifact is frozen, the controller may rerun `check_style_profile.py <finalized-draft> --draft-gate --strict --genre <selected-genre>` without `--repair-brief` for the full report. The script automatically uses `references/style-profile.json` from the installed skill unless `--profile` or `--corpus-dir` overrides it. A `revise` status means finalized repair failed. A missing profile makes the result `review`, not ready for blind rounds.
