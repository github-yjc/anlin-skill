...
2. **Finalized repair checkpoint**: copy the bounded checkpoint draft into a separate `finalized/` case directory, then start from that copy and its visible checker results. The generator-facing attempt is deliberately short: run the pre-write formal gates only if the checker path is already directly available, use the repair brief when available, make one complete source rewrite, persist `finalized/draft.md`, and stop. The controller then reruns hard gate and style-profile reports after the artifact is frozen. If it still does not pass, record unresolved repair-path evidence rather than letting the same agent keep editing around metrics. A second write/edit to `finalized/draft.md`, a post-write repair-agent gate loop, post-write `python -c` / `Measure-Object` / `wc` / local counter probes, or explicit threshold arithmetic in the same finalized attempt makes the trace invalid controller evidence even when the artifact changes. In short, a post-write repair-agent gate loop, or explicit threshold arithmetic is invalid; post-write local counters are the same loop in smaller form. Post-write metric probes are invalid because the repair agent is no longer allowed to validate or tune the frozen artifact; only the controller does that. A controller may start a new separately logged attempt for diagnosis, but the current finalized attempt remains non-pass. Do not read or grep checker source, test files, or hidden threshold constants. Do not glob or list checker source, parent skill directories, or old logs either. A finalized run that inspects `check_anlin_violations.py`, `check_style_profile.py`, `clean_run_checker.py`, `test_anlin_tooling.py`, or threshold names such as `YELLOW_REVIEW_FAMILY_THRESHOLD` is contaminated even if the final draft later passes. Save the final article and full validation reports separately from the bounded checkpoint. This checkpoint answers: can the skill plus its public repair interface produce a usable final article without metric chasing or source inspection?
