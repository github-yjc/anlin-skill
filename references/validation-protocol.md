...
- A nonzero controller `--strict --repair-brief` result inside the prepared brief normally means the draft did not pass the profile gate and should be repaired by writing a complete revised `finalized/draft.md`. It is not by itself a tool failure. The repairing agent should not respond by running gates, reading hidden thresholds, printing a proposed final article only in the terminal, or continuing to analyze full metrics.
