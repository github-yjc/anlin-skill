...
- For bounded clean-eval, create an empty `.anlin-clean-eval-mode` marker in the case workspace or otherwise explicitly select clean-eval mode without adding style hints. The marker selects the two-actual-checker-call protocol; preflight attempts are separate and do not consume those calls. It must not contain prompts, rubrics, examples, or failure analysis.
...
Generator-side preflight is allowed only through `clean_run_checker.py`. `CLEAN_RUN_PREFLIGHT` means the draft was not ready and no formal checker call was consumed; repeated preflight results do not exhaust the two-call budget. `CLEAN_RUN_POSTCHECK_PREFLIGHT` means checker call 1/2 has run, but the current repaired draft still has a known underbuilt source or hard shape failure before checker call 2/2; it also consumes no formal checker call and allows only the one bounded source-replacement or explicit shape-repair action named by the wrapper. The wrapper does not rewrite `draft.md`; stage snapshots preserve the submitted bytes. `CLEAN_RUN_PREFLIGHT_STOP` means the generator did not reach a checker-ready article within the bounded preflight attempts; record the run as invalid or failed and do not let the same generation agent continue repairing indefinitely. This preserves the distinction between clean-eval mode and ordinary user mode.
...
The first checkpoint is deliberately bounded: it is the result after natural source guidance and limited checker-driven repair, frozen at the clean-eval boundary. Within that checkpoint, distinguish four signals: first-submission source guidance, preflight readiness before any formal checker call, post-check preflight readiness before checker call 2/2, and the two-call checker boundary if the draft reaches it. A run stopped at `CLEAN_RUN_PREFLIGHT_STOP` before call 1/2 is a source/preflight failure with no evidence that the two actual checker calls were tested. A run stopped by post-check preflight after call 1/2 has tested the first checker plus limited repair, but not call 2/2. A run with `calls=2` tests the limited checker repair boundary. The second checkpoint is deliberately controlled: it starts from a copied bounded draft and tests whether the public finalized repair interface can produce one persisted source rewrite that the controller can validate. These answer different failure questions. A clean finalized draft cannot retroactively make the bounded draft a success, and a failed finalized draft means the final article itself is still unresolved.
...
2. **Finalized repair checkpoint**: copy the bounded checkpoint draft into a separate `finalized/` case directory. Before the repair-agent attempt, the controller may run `scripts/prepare_finalized_repair_brief.py <case-dir>/finalized/draft.md --genre <selected-genre>` to write `finalized/repair-brief.txt` from strict hard-gate blockers and the compact style-profile repair brief. A valid hard-gate failure produces `repair_mode: source_rewrite_compact`: the brief exposes one source focus and the route-appropriate scope—usually one local cluster, but an explicitly severely underbuilt draft may require a source-level rebuild across its existing movements—while keeping the detailed blocker/profile list in controller evidence. A hard-pass profile review may produce `repair_mode: hard_pass_review_in_place`, which is also brief-only. For either compact mode, the repair agent reads `draft.md` plus `repair-brief.txt` only and does not load the finalized minimum or reconstruct hidden metrics. The generator-facing attempt is deliberately artifact-only: the repair agent makes one complete source rewrite, persists `finalized/draft.md`, and stops. The repair agent must not run local gates, controller helpers, counters, path probes, source/test reads, threshold searches, or old-log searches; any `check_anlin_violations.py`, `check_style_profile.py`, `clean_run_checker.py`, or `prepare_finalized_repair_brief.py` command in the repair-agent trace contaminates the attempt. The controller then reruns hard gate and style-profile reports after the artifact is frozen. If it still does not pass, record unresolved repair-path evidence rather than letting the same agent keep editing around metrics. A second write/edit to `finalized/draft.md`, a post-write repair-agent gate loop, post-write `python -c` / `Measure-Object` / `wc` / local counter probes, or explicit threshold arithmetic in the same finalized attempt makes the trace invalid controller evidence even when the artifact changes. In short, the repair agent writes the artifact; only the controller validates it. A controller may start a new separately logged attempt for diagnosis, but the current finalized attempt remains non-pass. Save the final article and full validation reports separately from the bounded checkpoint. This checkpoint answers: can the skill plus its public repair interface produce a usable final article without metric chasing or source inspection?
...
- Before the repair attempt, the controller may prepare `repair-brief.txt` with `prepare_finalized_repair_brief.py`; this helper runs the hard gate and `check_style_profile.py --repair-brief` outside the repair-agent trace. The repair brief is the generator-facing interface. For a valid hard-gate failure it now uses one `source_rewrite_compact` focus/scope envelope rather than exposing a blocker checklist; the scope is route-specific, so a severe underbuilt draft may authorize a source-level rebuild while an ordinary or overfull route stays local. For hard-pass profile review it may use `hard_pass_review_in_place`. Both keep full profile metrics controller-only so the repairing agent does not optimize every ratio one by one or save a dense 8-25-row prose repair. After one source rewrite, the repair agent stops. After the finalized artifact is frozen, the controller reruns hard gate and runs `check_style_profile.py <finalized-draft> --draft-gate --strict --genre <selected-genre>` without `--repair-brief` for the full report. The script automatically uses `references/style-profile.json` from the installed skill unless `--profile` or `--corpus-dir` overrides it. A `revise` status means finalized repair failed. A missing profile makes the result `review`, not ready for blind rounds.
- Only a schema-valid controller `--strict --repair-brief` result with return code `1` means the draft did not pass the profile gate and should be repaired by writing a complete revised `finalized/draft.md`. `controller_tool_error`, invalid output, or any other unexpected result is unavailable, not quality evidence. The repairing agent should not respond by running gates, reading hidden thresholds, printing a proposed final article only in the terminal, or continuing to analyze full metrics.
...
- If finalized repair improves one rhythm metric by creating the opposite failure, such as period grid becoming comma drag, 80+ uniform lines becoming 30-40 compressed prose lines, 8-25 dense prose rows, or refusal consequence becoming room/object texture, classify it as unresolved repair-path drift. A single corridor reset can be useful inside the one source rewrite, but do not continue metric ping-pong after the validation rerun.
...
- bounded fails, finalized passes: strengthen Layer 0/Layer 1 generation guidance and clean-eval repair instructions; the public repair interface can recover, but the source loop is still weak.
