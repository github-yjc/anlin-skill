...
- Own the clean-eval first-draft source loop so the generator does not need long repair or validation files before writing. `clean-generation-brief.md` is the expanded repair contract after the wrapper reports findings, not the first-draft entry point.
...
- finalized repair checkpoint: a copied draft in a separate `finalized/` directory, measuring whether the checker and repair references can converge without mutating the bounded result. The generator-facing finalized attempt is intentionally short: at most one pre-write repair brief, one complete source rewrite persisted to `finalized/draft.md`, then stop. The repair brief should stay shape-first and source-action oriented; full profile metrics remain controller evidence after the artifact is frozen. Terminal/log-only final prose is an artifact failure. A dense 8-25-row standard repair is still prose-block drift, not a valid line-broken article. The opposite legal-looking failure is a 45-70-line caption grid with no row visibly longer than about 24 Chinese characters; this is still line-rhythm drift, not a pass. A second `Write draft.md` or `Edit draft.md`, a post-write repair-agent gate loop, post-write python -c/Measure-Object/wc counters, `Test-Path`/glob/list path probing, or visible threshold arithmetic in the same finalized attempt is also invalid controller evidence. The selected genre must be frozen from the case/prompt/bounded draft before repair and passed to both hard-gate and style-profile validation when available; normal checker success alone is not a finalized pass. The repairing agent should use style-profile `--repair-brief` as the generator-facing interface, while the controller reruns hard gate and the full style-profile report after the artifact is frozen. The repairing agent may use pre-write gate outputs, but must not read or grep checker source, tests, hidden threshold constants, parent skill directories, old logs, or run local counters after writing; it must not glob or list those paths either. A finalized pass after source/threshold inspection or write-after-counting inspection is contaminated repair evidence and should not be counted as clean convergence.
...
Standard-diary punctuation drift now has an explicit source failure name: punctuation pendulum. If a repair swings from comma-drag to period-row grid, or from period-row grid to one huge comma chain, Layer 0 did not build breathing clusters. The first-draft fix belongs in `clean-eval-first-draft-minimum.md` and `standard-diary-source-engine.md`; detailed repair guidance remains in `clean-generation-brief.md`. Unfinished action, reply, payment, door/body movement, and short failure drops decide the marks. Layer 4 may warn with `标准日寄句号网格`, `逗号密度过高`, red `punctuation`, or `period_row_grid`, but the detector is not the fix.
