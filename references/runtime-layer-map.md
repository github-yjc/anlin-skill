...
This file is for maintainers, auditors, and failed-run diagnosis. Do not load it during ordinary article generation unless the user asks how the skill is organized.
...
- Tell the agent the goal, output rules, clean-run boundary, and the smallest writing surface.
...
These files may guide drafting, but they must not become visible subject matter. A negative example about AI, game, background, or detection is not a scene prompt.
...
These are not first-draft files in formal clean generation. Opening too many of them before writing increases checklist pastiche, background stuffing, and prompt-shaped completeness.
...
This layer can fail a draft. It should not be handed to the generator as extra style advice during a formal clean run.
