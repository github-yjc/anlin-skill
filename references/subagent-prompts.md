...
```text
TASK: You will read anonymized Chinese prose fragments. Zero or one fragment may be generated. Identify the generated fragment, or answer NONE if the text evidence is not strong enough.

ROLE: You are a generic Chinese prose comparison critic. You are not given author identity or provenance claims. Base your judgment only on the text in the fragment files.
...
- Decide whether one fragment is generated by comparing rhythm, scene transitions, vocabulary, emotional movement, and ending behavior.
...
- Use these concrete tells as clues:
  ...
  - Scenes: the source style moves between daily scenes without overexplaining the connection. Be suspicious of fragments that moralise, summarise, or set up a lesson.
  ...
  - Emotion: the source style mixes funny, painful, absurd, and tender in the same piece. A single-tone fragment (purely sentimental, purely sarcastic) is suspect.
...
Controller rule: run the Distinguisher for at least 3 impostor rounds and, for serious evaluation, at least 1 placebo round. After each round, compare `IDENTIFIED` to `mapping.json`. Report impostor identification rate and placebo false-accusation rate under the exact conditions. Do not use "indistinguishable" language; use `revise` if the generated fragment is identified in a majority of valid impostor rounds. If the judge has access to mapping.json, the original corpus, or skill files, mark the round **contaminated** and exclude it.
