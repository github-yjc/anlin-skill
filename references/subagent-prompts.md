...
INPUTS: The controller will give you a directory containing files named fragment-01.txt through fragment-NN.txt. Each file is similar length. Exactly one file contains a generated draft.

MUST DO:
- Read ONLY the fragment-*.txt files in the provided directory.
...
- Do NOT try to read the skill files, mapping.json, or any file other than the fragment-*.txt files.
- Do NOT ask for the author name, generation prompt, or style rules.
- Do NOT infer from file names or order.

OUTPUT FORMAT (exactly):
Fragment-XX is the AI-generated one because: [your reasons].
```
