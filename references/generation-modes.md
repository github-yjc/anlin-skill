...
- after the second checker call, stop all repair work. Checker findings are not tool failures. Do not edit, write, compare, explain, invoke fallback, or run another command. You may read `draft.md` once only to output it exactly. Output the current `draft.md` content exactly, even if the second checker still has errors. A third checker call, post-second write, or unpersisted manual repair invalidates the clean test more than the remaining errors do.
