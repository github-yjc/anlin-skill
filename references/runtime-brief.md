...
For mother/family sincere pieces, do not choose the memory first. Choose one present-action anchor first, then let one memory prop attach to it by accident. A usable anchor changes what the narrator does next: a sink, bowl, door, neighbor, call, unsent reply, wet clothing, wrong slipper, body itch/noise, room chore, or small object failure. A prompt prop such as eggs, rain, holiday feed, or blessing is not an anchor until it creates a present failure. If the opening can be summarized as `today is the holiday, I saw eggs, I remembered rain`, the source is already too clean; restart from today's failed action and let the mother material enter late or partially.
