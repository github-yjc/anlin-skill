...
Generated-draft AI slop includes explanatory `不是X，是Y` / `不是X，而是Y` / `不是X。是Y`, `像X，其实不是，好像就是Y`, blog-like insight translation, self-annotation such as `其实就是那种怎么说呢` / `大概就是这个意思`, unsupported specific place names, unsupported game-role filler, isolated Chinese punctuation lines such as `。`, ordinary `——` connectors, literary `——那种...` explanation, abstract emotion labels such as `放松` / `释然` / `自洽`, A/B or 甲乙 variable explanation, or any line that mainly announces a reframe rather than letting a scene create it.

Use at most one checker-driven repair loop and call `scripts/clean_run_checker.py draft.md --strict --draft-gate` at most twice. The wrapper delegates to the normal checker and records the clean-eval count. Calling the wrapper commits the current draft to clean-eval protocol; do not later switch the same file back to ordinary mode. If it prints `CLEAN_RUN_PREFLIGHT`, the draft is too short, too overfilled, compressed into prose blocks, or missing minimum connector, engine, or coarse self-damage signals; keep writing concrete action, body, social, and off-axis material when short, or cut unsupported/non-consequential texture when long, then run the wrapper again. That preflight is not a checker call and appears only before the first actual checker call. If it prints `CLEAN_RUN_PREFLIGHT_STOP` or `CLEAN_RUN_STOP`, treat that as `FINAL BOUNDARY`: do not write `draft.md`, do not repair, do not switch to `check_anlin_violations.py`, and do not run another rhythm script in the same directory. The next tool action must be reading `draft.md` once and outputting it unchanged so the controller can mark the run invalid or failed. Checker findings are not tool failures; a nonzero checker exit with findings is an ordinary evaluation result. Treat the second checker as the final exam, not another brainstorming step. Before running it, inspect `draft.md` itself for the repeat failures that generated models often miss: too few different natural high-frequency connectors, the same connector such as `其实/已经/当时` carrying many transitions, fewer than three paragraph-engine signals, isolated `。` lines, first-20-line comma ratio near zero, visible line grid, visible long prose blocks, fewer than 45 body lines, and body length shrinkage. If a content rewrite happens after `split_long_lines.py`, the split no longer applies; run `scripts/split_long_lines.py draft.md --in-place --target-lines 58` again before the second checker whenever the current file is under 45 body lines or still has long prose blocks. If `行末逗号比例` fired on the first checker, or the rewritten file still closes nearly every early line with `。`, run `scripts/soften_line_endings.py draft.md --in-place` before the second checker; it only changes eligible line-final `。` to `，` and does not add content. A third checker call, post-second write, fallback rewrite, or unpersisted final repair invalidates the clean-eval run more severely than a weak article does. After the second checker call, stop repair work: do not edit, write, compare, explain, invoke fallback, or run another command. The next tool action may only be reading `draft.md` once to output it exactly. Output the current `draft.md` content exactly, even if errors remain. Do not produce an unpersisted hand-repaired final variant after the second checker. Soft style warnings are not permission to run a third repair.
