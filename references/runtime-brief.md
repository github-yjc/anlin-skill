...
For illness, pain, insomnia, gout, stomach, urine, tooth, sweat, or similar body prompts, avoid the private case-report chain: symptom inventory -> search result -> food taboo -> refrigerator inventory -> ambient sound. Use the body as an interruption that changes the next action, then move away from the diagnosis. A useful repair usually removes one repeated symptom/food/screen packet and replaces it with a practical or social consequence: a failed walk, a spilled cup, a bad reply, a neighbor or cashier seeing the maneuver, a bathroom/slipper problem, or an errand that the body ruins. Do not add more symptoms only because roughness is missing.
...
For illness-search prompts, treat medical pages and forum claims like other screen surfaces. If the user gives `有人说痛风是富贵病`, do not preserve `有人说` as narration. Make it one cropped screen phrase or article title, then let the hand, room, food, route, or another person answer it. If a repair still repeats the illness word, body part, food object, and diagnosis label every few lines, delete one full packet before adding any new consequence.
