...
When `ngram_texture` is red or a local packet repeats, do not keep the same scene and swap adjectives. Repeated packets often look like `hand/gray/dirt`, `white food/cutting board/knife`, `garbage/bin`, `screen/room/dark`, or `money/payment/coin` coming back in the same order. Cut one packet whole. Replace it with a different engine type: someone reacts, a reply fails, money is handled awkwardly, the route changes, a small errand interrupts, or a practical object forces the narrator to move. The fix should reduce repeated nouns and increase consequence, not raise texture density.
