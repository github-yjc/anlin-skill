...
Use at most one checker-driven repair loop and call `scripts/clean_run_checker.py draft.md --strict --draft-gate` at most twice. The wrapper delegates to the normal checker and records the clean-run count. If it prints `CLEAN_RUN_PREFLIGHT`, the draft is too short or missing minimum connector, engine, or coarse self-damage signals; keep writing concrete action, body, social, and off-axis material, then run the wrapper again. That preflight is not a checker call and may appear before call 1/2 or 2/2. If it prints `CLEAN_RUN_STOP`, immediately read `draft.md` once and output it unchanged. Checker findings are not tool failures; a nonzero checker exit with findings is an ordinary evaluation result. Treat the second checker as the final exam, not another brainstorming step. Before running it, inspect `draft.md` itself for the repeat failures that generated models often miss: fewer than five natural high-frequency connectors, fewer than three paragraph-engine signals, isolated `。` lines, first-20-line comma ratio near zero, visible line grid, visible long prose blocks, fewer than 45 body lines, and body length shrinkage. If a content rewrite happens after `split_long_lines.py`, the split no longer applies; run `scripts/split_long_lines.py draft.md --in-place --target-lines 58` again before the second checker whenever the current file is under 45 body lines or still has long prose blocks. If `行末逗号比例` fired on the first checker, or the rewritten file still closes nearly every early line with `。`, run `scripts/soften_line_endings.py draft.md --in-place` before the second checker; it only changes eligible line-final `。` to `，` and does not add content. A third checker call, post-second write, fallback rewrite, or unpersisted final repair invalidates the clean-generation run more severely than a weak article does. After the second checker call, stop repair work: do not edit, write, compare, explain, invoke fallback, or run another command. The next tool action may only be reading `draft.md` once to output it exactly. Output the current `draft.md` content exactly, even if errors remain. Do not produce an unpersisted hand-repaired final variant after the second checker. Soft style warnings are not permission to run a third repair.
