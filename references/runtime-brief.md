...
Use at most one checker-driven repair loop and call the checker at most twice. Checker findings are not tool failures; a nonzero checker exit with findings is an ordinary evaluation result. Treat the second checker as the final exam, not another brainstorming step. Before running it, inspect `draft.md` itself for the repeat failures that generated models often miss: fewer than five natural high-frequency connectors, fewer than three paragraph-engine signals, isolated `。` lines, first-20-line comma ratio near zero, visible line grid, and body length shrinkage. If `行末逗号比例` fired on the first checker, run `scripts/soften_line_endings.py draft.md --in-place` before the second checker; it only changes eligible line-final `。` to `，` and does not add content. A third checker call, post-second write, fallback rewrite, or unpersisted final repair invalidates the clean-generation run more severely than a weak article does. After the second checker call, stop repair work: do not edit, write, compare, explain, invoke fallback, or run another command. The next tool action may only be reading `draft.md` once to output it exactly. Output the current `draft.md` content exactly, even if errors remain. Do not produce an unpersisted hand-repaired final variant after the second checker. Soft style warnings are not permission to run a third repair.
...
After the first checker, repair by replacement rather than subtraction. If you delete a comment chain, unsupported game detail, city, dash sentence, or prompt-shaped line, add an equally concrete action, body consequence, social misfire, route, food, household object, or app surface before the second checker. If the checker reports prose compression, long-line density, or too few body lines, run `scripts/split_long_lines.py draft.md --in-place --target-lines 58` before rewriting content; it only breaks existing long lines at Chinese punctuation. If the checker reports line-grid errors, do a rhythm rewrite: run `scripts/merge_short_lines.py draft.md --in-place --target-lines 68`, then reduce toward 55-70 body lines, merge many short rows into longer action/speech/thought lines, keep at least six 24+ character lines, and remove all-current-match game details. If it reports `行末逗号比例`, run `scripts/soften_line_endings.py draft.md --in-place` or manually convert non-final complete-sentence lines into comma continuation where the next line actually continues the action or thought. Never call the second checker on a draft that became visibly shorter than the first candidate, still has a visible short-line grid, still has long-line prose blocks, or still closes nearly every early line with `。`.
