...
Latest finalized-repair routing fix: after a social-decline draft writes back successfully but still fails because the wedding/refusal line sits beside room, screen, water, or route texture, the compact repair brief now routes the primary action to `rebuild_refusal_aftermath_engine` before period-grid repair. This is meant to stop the repair agent from treating a refusal-engine failure as a punctuation task. A targeted retest confirmed that this can clear the earlier social-refusal and period-grid blockers, but it also exposed a new false escape: the repair can shrink into a 568-character short summary. The brief now says the deleted private texture packet must be replaced with refusal-coupled consequence movement and should preserve a complete standard-diary corridor, usually 900+ body Chinese characters. The controller summary also recognizes OpenCode JSONL write-tool mutations regardless of whether `content` or `filePath` appears first in the event payload. This is not blind-round evidence; it narrows the next retest question to whether the short brief can preserve article mass while making the refusal aftermath change the next visible action.
