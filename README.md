...
The latest severe-underbuilt bounded retest was deliberately excluded from quality evidence: the generator was allowed to combine a whole-source rebuild with a rhythm-script action and then rewrote after the script, so the trace was invalid. The generator-facing contract now keeps severe source repair source-only and defers rhythm tooling to a later wrapper call; this is a protocol/interface correction, not a quality or blind-readiness claim.
