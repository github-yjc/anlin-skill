...
Latest finalized-repair routing fix: after a social-decline draft writes back successfully but still fails because the wedding/refusal line sits beside room, screen, water, or route texture, the compact repair brief now routes the primary action to `rebuild_refusal_aftermath_engine` before period-grid repair. This is meant to stop the repair agent from treating a refusal-engine failure as a punctuation task. It is not blind-round evidence; it only narrows the next retest question to whether the short brief can make the refusal aftermath change the next visible action while preserving line rhythm.
