...
Latest clean-eval routing/source fix: targeted source-generation retests showed two separate boundaries. First, a generator could misread a formal workspace as ordinary article generation even when `.anlin-clean-eval-mode` existed; the runtime router now makes the marker outrank ordinary article wording, and `runtime-brief.md` / `anti-ai-slop.md` send misrouted formal workspaces back to `clean-eval-first-draft-minimum.md` before the first `draft.md`. Second, the social-decline source guidance now moves refusal aftermath away from pure phone/screen loops: recent retest output did create a post-refusal non-screen action through delivery-door contact, unpaid transfer, balance, and old-debt residue. It still stopped at preflight before checker call 1/2 because the first saved article was written as five dense prose body lines and then cut by rhythm tooling, with connector turns still absent. Current source guidance therefore rejects 5-7 long-paragraph first drafts and says the file-write `content` itself must already show broken body rows before the first wrapper call. This is source-shape repair, not blind-round evidence.
