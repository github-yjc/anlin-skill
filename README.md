...
The skill is not yet proven to meet the `<=10%` stable impostor-identification target. The latest documented clean-eval cases still show bounded-generation failures before blind rounds: one case remained a source/checker systemic gap, and the later family-dinner case exposed a preflight boundary that stopped a 946-character draft before formal checker call 1/2. No current report yet contains 15 clean-eval generation runs plus calibrated `3 impostor + 1 placebo` style rounds sufficient to claim the target.
