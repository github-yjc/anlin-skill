...
Follow-up retest `iteration-20260705-12/eval-03-valentines-alone` used another rotated model surface with thinking enabled after commit `a917146`. The trace was clean and respected the preflight stop boundary (`calls=0`, `preflights=3`), so the bounded checkpoint is a valid source-guidance failure. The final bounded draft had 963 body Chinese characters and 45 body lines after a rhythm reset, but strict controller validation still reported `高频词覆盖不足`, `粗粝自毁信号不足`, `段落发动机信号偏弱`, and clean-eval stop boundary; style-profile stayed `revise` with red `connectors`, `line_rhythm`, and `punctuation` plus broad yellow drift. Manual root review found two model-agnostic causes: social-feed prompts still encouraged a feed montage (`多条朋友圈/点赞取消/最新视频`) instead of one screen surface that changes action, and wrapper preflight text still tempted the generator to repeat diagnostic metrics as a checklist. Root fixes from this case: Layer 0 now treats 朋友圈/short-video/social comparison as one pressure surface before the first checker, not a scene slate, and the clean-run wrapper now explicitly tells generation agents not to summarize, quote, or enumerate diagnostics as a TODO list.
