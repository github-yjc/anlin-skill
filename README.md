...
The active standard-diary source contract is now fragment-slate based. A standard article normally moves through several independent thought-turns connected by earned association, contrast, memory, echo, time jump, self-correction, or direct voice jump; one continuous scene is a deliberate exception, not the default. This qualitative guidance has no fragment-count quota. Iteration-164 produced a complete bounded artifact and stable controller hashes, but it is now classified as `invalid-controller-evidence`: the wrapper named a rhythm script while its old static `next_action` simultaneously told the generator to write and rerun the wrapper immediately, so the named rhythm action was skipped. The trace checker now reports that skip as an error. The article's source movement improved, but its quality findings cannot be promoted to a valid bounded candidate from a contaminated sequence. No finalized, blind, placebo, or recognition-rate evidence was added by that run.

Iteration-165 did execute the first named rhythm action in order: one content write, the exact in-place rhythm script, then the wrapper. A later source-only action contained one failed `apply_patch` attempt that did not persist, a read of the current draft, one completed write, and the immediate wrapper rerun. Once the trace parser retained tool status, the saved trace had zero protocol errors and only the provider-visible planning warning; this case is process-valid but quality-invalid, not `invalid-controller-evidence`. Do not start finalized repair from this case. Controller-only diagnosis, which does not count as pass evidence, shows directional source progress but no quality pass: the first submission used several thought-turns instead of one scene, yet remained a 698-character/13-row prose block with AI-reframe and polished-simile findings; the frozen 766-character/35-row artifact had one strict length-buffer error and style-profile `review` with red connectors/punctuation. The matched standard corpus has median 811 body characters and 21/34 originals below 900, so the universal 900-character generated-draft buffer needs a separate calibration study before another source prompt lock, threshold change, fresh bounded case, or finalized repair.
...
The severe-underbuilt bounded retest in iteration-160 was deliberately excluded from quality evidence: the generator was allowed to combine a whole-source rebuild with a rhythm-script action and then rewrote after the script, so the trace was invalid. Fresh iterations 161-163 then isolated a Layer 0/1 language-interface conflict: 161 ended as a 722-character prose block, 162 as a 521-character short-line grid, and 163 as a 670-character/90-line bare-row grid already present in the first submission. Iteration-164 moved the source toward a real collage but exposed a second protocol defect in the named rhythm sequence. The current interface separates source-only, mixed source-plus-shape, pure-shape, and stop actions; active source docs also state that punctuation inside a long row is not lineation, natural continuation must be visible without naked caption rows, and every visible row must not be sealed with a period. These remain source/protocol checkpoints, not quality or blind-readiness claims.
