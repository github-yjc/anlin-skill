...
The skill is not yet proven to meet the `<=10%` stable impostor-identification target. The latest documented two-checkpoint case is still `systemic_gap`: bounded clean-eval stopped at preflight, and finalized repair cleared hard errors but remained style-profile `review` because five independent yellow drift families persisted. No current report yet contains 15 clean-eval generation runs plus calibrated `3 impostor + 1 placebo` style rounds sufficient to claim the target.
