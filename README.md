...
The latest documented boundary is preserved in `references/development-log.md`. In short: the compact `--repair-brief` and inline finalized fallback fixed the specific terminal-only / unchanged-artifact repair failure, but the newest targeted retests still did not become blind-round-ready. Rechecking the latest social-decline draft under the current gates narrows the remaining hard failures: bounded generation still stops before checker call 1/2 with rhythm/punctuation problems, and finalized repair can still drift into source-level rhythm/punctuation loops. The current repair interface therefore treats a finalized attempt as one source rewrite plus one validation rerun; repeated metric bounces are unresolved evidence, not a reason to continue local edits. A second write/edit to `finalized/draft.md`, a third formal gate sequence, or visible threshold arithmetic in the same finalized attempt is invalid controller evidence. Recent single-write retests improved trace discipline and removed the message-order glue failure, but the drafts still failed quality with rough self-damage shortage, period-row grid, prose-block compression, equal short-sentence row grids, or an unsupported half-day-leave / shift-schedule chain created from a refusal excuse. The current interface now makes hard-gate findings outrank the compact style brief, treats the single finalized write as atomic, rejects prose-block compression as a false escape from period-grid failure, clarifies that private room-only wet texture is not rough exposure, and keeps `忙项目`-style refusal excuses from becoming leave, 调休, 排班, or office biographies. Do not expand to blind rounds until a fresh bounded/finalized case is pass-ready under the active protocol.
...
Use `--repair-brief` only as the generator-facing repair interface. It intentionally hides full metric lists so the repair agent writes a complete revised `draft.md` instead of reasoning through every ratio. After the artifact is frozen, the controller reruns `check_style_profile.py` without `--repair-brief` for the full audit report.
